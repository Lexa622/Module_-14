...
cursor.execute("DELETE FROM Users WHERE id = ?", (6,))  # удаление данных
cursor.execute("SELECT COUNT(*) FROM Users")    # счетаем количество элементов ( строк )
total_users = cursor.fetchone()[0]      # сохраняем количество элементов
cursor.execute("SELECT SUM(balance) FROM Users")    # сумма балансов
all_balances = cursor.fetchone()[0]     # сохраняем сумму балансов
print(all_balances / total_users)       # средний баланс выводив в консоль
